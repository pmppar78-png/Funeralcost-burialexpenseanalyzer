...
// ── Generate extensionless → .html canonical redirects ─────────
// Netlify serves every .html file at its bare, extensionless path with a
// 200 as well (e.g. /about AND /about.html both return the same page).
// Because each page's canonical points at the .html URL, Google buckets
// the extensionless variant as "Alternate page with proper canonical tag".
// To remove the duplicate URL entirely we 301 each extensionless path to
// its .html canonical. The list is derived from allSitemapPages so it
// always matches exactly the set of indexable pages. index.html is
// skipped (the homepage is served at "/" and /index.html already 301s to
// "/" via netlify.toml); these rules are appended to the redirect engine
// and do not conflict with the disjoint vanity aliases in netlify.toml.
const redirectRules = allSitemapPages
  .filter(p => p !== 'index.html' && p.endsWith('.html'))
  .map(p => `/${p.slice(0, -5)}    /${p}    301!`);
const redirectsContent =
  '# Auto-generated by generate-pages.js — do not edit by hand.\n' +
  '# 301 each extensionless path to its .html canonical to prevent\n' +
  '# duplicate (alternate-canonical) URLs in search engines.\n' +
  redirectRules.join('\n') + '\n';
fs.writeFileSync(path.join(OUT, '_redirects'), redirectsContent);
console.log(`  Redirects: _redirects (${redirectRules.length} extensionless → .html)`);
